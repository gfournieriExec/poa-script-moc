...
async function generateInitialKey(_testKeysManagerAddress) {
  ...
  let KeysManagerAddress = _testKeysManagerAddress
    ? _testKeysManagerAddress
    : config.Ethereum.contracts.KeysManager.addr;
  ...
  let keysManager;
  try {
    keysManager = await new web3.eth.Contract(
      KeysManagerAbi,
      KeysManagerAddress
    );
  } catch (err) {
  ...
  for (let i = 1; i < 5; i++) {
    ...
    const keyStoreFileName =
      constants.outputFolderPath + "validator-" + i + ".json";
    const passwordFileName =
      constants.outputFolderPath + "validator-" + i + ".key";
    const content = JSON.stringify(keyObject);

    try {
      await saveToFile(keyStoreFileName, content);
    } catch (err) {
      return errorFinish(err);
    }
    console.log(
      `Initial key ${initialKey} is generated to ${keyStoreFileName}`
    );

    try {
      await saveToFile(passwordFileName, password);
    } catch (err) {
      return errorFinish(err);
    }
    ...
    try {
      await addInitialKey(keysManager, initialKey, MoC);
    } catch (err) {
      return errorFinish(err);
    }
  }
